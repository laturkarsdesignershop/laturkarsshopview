 ...
 * input  : mysqli connection, raw keyword
 * working: creates safe SQL LIKE condition for order id, customer name & numbers
 * output : SQL condition string
 */
function buildSearchCondition($conn, $searchKeyword) {
    if (empty($searchKeyword)) return "1=1";

    $kw = "%" . $conn->real_escape_string($searchKeyword) . "%";

    return "(
        o.id_order LIKE '$kw'
        OR c.fullname LIKE '$kw'
        OR c.phone LIKE '$kw'
        OR c.mobile LIKE '$kw'
    )";
}
...
// }
    ...
    $searchCondition = buildSearchCondition($conn, $searchKeyword);

    $sqlCount = "
        SELECT COUNT(*) AS total
        FROM tailor_order o
        LEFT JOIN tailor_customers c ON o.id_customers = c.id_customers
        WHERE $searchCondition AND o.status != 'Deleted'
    ";

    $countResult = executeQuery($conn, $sqlCount, false, true);
    $totalFiltered = intval($countResult['total'] ?? 0);

    $sqlPaged = "
        SELECT o.*, 
               COALESCE(c.fullname,'Unknown') AS customer_name,
               COALESCE(c.phone,'') AS customer_phone,
               COALESCE(c.mobile,'') AS customer_mobile
        FROM tailor_order o
        LEFT JOIN tailor_customers c ON o.id_customers = c.id_customers
...
<form method="POST" class="mb-3 flex flex-col sm:flex-row sm:items-center sm:space-x-2 w-full max-w-xl">
    <input type="text" name="searchKeyword" placeholder="Search Order ID / Customer Name / Mobile..." value="<?php echo htmlspecialchars($searchKeyword); ?>" 
